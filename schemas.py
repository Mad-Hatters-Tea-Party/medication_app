...
class UserCreate(BaseORMModel):
    ...
    user_weight: Optional[int] = Field(None, gt=0)  # Weight must be greater than 0
     ...
     # Custom validator to handle the dob field
    @field_validator('user_dob', mode='before')
    def check_and_parse_dob(cls, v):
        if isinstance(v, str):
            # Manually convert the string to a date
            try:
                v = datetime.strptime(v, "%Y-%m-%d").date()  # Convert string to date
            except ValueError:
                raise ValueError("Invalid date format for user_dob. Expected format: YYYY-MM-DD.")
        elif v is None:
            raise ValueError("user_dob is required.")

        # Now v is guaranteed to be a date object
        age = calculate_age(v)
        if age < 18:
